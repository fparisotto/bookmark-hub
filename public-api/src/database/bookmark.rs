...
#[instrument(skip(db))]
pub async fn get_by_user(db: &Pool<Postgres>, user_id: &Uuid) -> Result<Vec<BookmarkWithUserData>> {
    const SQL: &str = r#"
    select
        b.*,
        bu.user_id,
    ...
    let result: Vec<BookmarkWithUserData> = sqlx::query_as(SQL).bind(user_id).fetch_all(db).await?;
    Ok(result)
}
